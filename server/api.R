...
sbpla_get_app <- function(token = tk, user_name, proj_name) {
  a <- suppressMessages(Auth(platform = "aws-us", token = token))
  ...
  list(
    "id" = sapply(wf_list, "[[", "id"),
    "name" = sapply(wf_list, "[[", "name")
  )
}
...
get_list_task_sbgapi <- function(platform_target, token, project_target) {
  ...
  if (length(tsk) > 1) sapply(tsk, "[[", "name") else tsk$name
}
...
get_list_app_sbgapi <- function(platform_target, token, project_target) {
  ...
  if (length(ap) == 1) {
    return(sapply(strsplit(ap$id, split = "/"), "[[", 3))
  }
  if ((class(try(ap@response, silent = TRUE)) == "try-error")) {
    return(NULL)
  }
  if (ap@response$status_code == 200) return(sapply(strsplit(sapply(ap, "[[", "id"), split = "/"), "[[", 3)) else return(NULL)
}

# list bco workflows of a selected project
...
get_app_cwl_sbgapi <- function(platform_target, project_target, app_target, token) {
  ...
  app_target_id <- if (length(ap) > 1) ap[[which(sapply(strsplit(sapply(ap, "[[", "id"), split = "/"), "[[", 3) == app_target)]]$id else ap$id

  # return a list of the workflow's CWL
  app_target_cwl <- httr::content(sevenbridges::api(base_url = .platforms[[platform_target]][["base_url"]], token = token, path = paste0("apps/", app_target_id, "/raw"), method = "GET"), as = "text", encoding = "UTF-8")
  class(app_target_cwl) <- "cwl"
  app_target_cwl <- read_cwl_json(app_target_cwl)
  app_target_cwl
}
...
upload_sbgapi <- function(platform, token, proj_name, file_name, file_path) {
  # print(paste(platform, token, proj_name, file_name, file_path, sep = ","))
  # auth connection
  a <- suppressMessages(Auth(platform = platform, token = token))
  # get projects
  p <- a$project(id = proj_name)
...
push_bco_to_git <- function(username, password, commit_text, filename_bco, file_bco, repository_url) {
  str_error <- ""
  flg_success <- TRUE
  # print(paste(username, "******", commit_text, filename_bco, file_bco, repository_url, sep = " | "))
  ...
  flg_clone <- tryCatch({
    ...
    # config settings
    git2r::config(repo, user.name = "BCO-app", user.email = "bco@sevenbridges.com")
  }, error = function(e) {
    print(e)
    FALSE
  })
