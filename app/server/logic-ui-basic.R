...
# prev, next buttons BASIC ------------------------------------------------

rv_page_2 <- reactiveValues(page_2 = 1)

observe({
  toggleState(id = "prev_btn_2", condition = rv_page_2$page_2 > 1)
  toggleState(id = "next_btn_2", condition = rv_page_2$page_2 < num_pages_basic)
  hide(selector = ".page_2")
  show(paste0("step", rv_page_2$page_2, "_2"))
})

nav_page_2 <- function(direction) {
  rv_page_2$page_2 <- rv_page_2$page_2 + direction
}

observeEvent(input$prev_btn_2, nav_page_2(-1))
observeEvent(input$next_btn_2, nav_page_2(1))

# prev, next buttons CWL ------------------------------------------------
