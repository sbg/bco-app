...
get_description <- reactive({
  ...
  biocompute::compose_description(
    ...
    load_desc_xref(), input$desc_platform,
    pipeline_meta, load_desc_pipeline_prerequisite(),
    load_desc_pipeline_input(), load_desc_pipeline_output()
  )
})
...
get_valSchema <- eventReactive(input$btn_upfile_validate_bco, {
  ...
  if (!is.null(input$upfile_validate_bco) || !(length(input$upfile_validate_bco) == 0)) {
    str_schema <- capture.output(biocompute::validate_schema(file = input$upfile_validate_bco$datapath))
  }
