...
tabPanel(
    ...
    fluidRow(column(
      ...
      shinydashboard::box(
        ...
        fluidRow(column(
          ...
          radioButtons("bco_op", NULL, choices = c("Create new BCO from a CWL workflow" = "bco_create"), inline = TRUE),

          selectInput(
            inputId = "plat",
            label = "Platform",
            choices = c("Seven Bridges Platform (US)" = "aws-us", "Seven Bridges Platform (EU)" = "aws-eu", "Cancer Genomics Cloud (CGC)" = "cgc", "DataSTAGE" = "f4c", "Cavatica" = "cavatica")
          ),
          conditionalPanel(
          ...
          conditionalPanel(
            ...
            tags$label(
              "Paste the auth token from the",
              tags$a(href = .platforms[["f4c"]][["token_url"]], "DataSTAGE developer dashboard", target = "_blank")
            )
          ),
          conditionalPanel(
          ...
          fluidRow(
            ...
            column(
              ...
              selectInput("task_name",
                label = "Choose a task of the project (Optional)",
                choices = NULL
              )
            )
          ),
      ...
      fluidRow(column(
        ...
        shinydashboard::box(
          ...
          fluidRow(
            column(
              width = 3, offset = 1,
              actionButton("btn_upload_plat", "Upload to Platform", icon = icon("cloud-upload-alt"), class = "btn btn-primary btn-block", style = "margin-left: 36px;")
            ),
            column(
              width = 3, offset = 0,
  ...
  tags$script(HTML("var header = $('.navbar > .container-fluid'); header.append('<div style=\"float:right;margin:10px;\"><a class=\"btn btn-default btn-sm\" href=\"./\" role=\"button\"><strong>Sign out</strong></a></div>'); console.log(header)"))
)
