...
The Genomics Compliance Suite (GCS) provides tools for generating, reviewing, and validating BioCompute Objects. The GCS can generate BioCompute Objects from user text input, workflows written in Common Workflow Language (CWL), and from CWL and task information generated on the Cancer Genomics Cloud. The GCS also include tools for reviewing and validating BioCompute Objects stored locally or on the cloud. Our goals in providing the GCS are to encourage further development of the standard, promoting the use of CWL to document complex bioinformatics workflows, and to facilitate the use of BCOs in operational settings; including developing the application to support FDA regulatory submissions. 

This user manual provides a brief introduction into BioCompute Objects, instructions for installing GCS, and an overview of GCS features. 
...
## Installing and Accessing the Genomics Compliance Suite

The GCS app can be deployed locally and on a remote server. Both the source code and a containerized (Docker) version is available on GitHub. The source code can also be executed from the Cancer Genomics Cloud, as well as any of the Seven Bridges Platforms, through the R Studio extension on out platforms. Additional GCS deployment details follow.

### Containerized app {#docker}

Running GCS from a containerize app will allow you to execute the app without having to install GCS requirements/dependencies on your local machine or server. Instructions for pulling, building, and running the Docker image can be found on the GCS GitHub page (https://github.com/sbg/gcs). The default username and password sevenbridges/sevenbridges (Figure 1).

### Cancer Genomics Cloud hosted app {#hosted}

The GCS can be executed from within the Cancer Genomics Cloud through R Studio. Execution instructions follow.
...
## Getting Started with the Genomics Compliance Suite {#start-page}

Executing the GCS will result in an optional login page or the GCS landing page depending on the GCS settings. 
...
### Landing Page
You will see the landing page of the GCS app after you login (See Figure 1).

![Landing Page of the Genomics Compliance Suite](assets/landing.png)

The navigation bar along the top of the screen provides access to primary GCS features including Generators and Utilities. Button along the bottom of the screen provide single click access to GCS BCO generators and to GCS help screen.

## Generating BioCompute Objects with GCS
...
After you click the "**Platform Composer**" page of the GCS platform under "**Generators**" tab, you will see the initial step of the BCO generation, import step (Figure 2).
...
![Example of Provenance Domain fields automatically filled by the GCS.](assets/step-provenance-1.png)
...
The GCS includes several BCO export features (Figure 11). A user can export a  BCO file in JSON file format, "**Export as JSON**". The generated BCO file is downloaded as a JSON file (Figure 12).
