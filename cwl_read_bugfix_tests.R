...
get_edges <- function (outputs, steps) {
  ...
  #get_edges() nested is_cwl_list() function - Can't seem to open and view default in package
  #making assumption based off is_cwl_dict
...
####load in other workflows with get_graph ####
  ...
  #neoepitode
  neoepitope <- read_cwl('workflow_examples/neoepitope-analysis-main-published.bco.json')
  ...
  #fusion transcript
  fusion_transcript <- read_cwl('workflow_examples/fusion_transcript_detection_chimerascan.json')

  get_graph(
    fusion_transcript %>% parse_inputs(),
    fusion_transcript %>% parse_outputs(),
    fusion_transcript %>% parse_steps()
  ) %>% visualize_graph() %>%
    visGroups(groupname = "input", color = "#E69F00", shadow = list(enabled = TRUE)) %>%
    visGroups(groupname = "output", color = "#56B4E9", shadow = list(enabled = TRUE)) %>%
    visGroups(groupname = "step", color = "#009E73", shadow = list(enabled = TRUE)) %>%
